...
const BOOKING_RATING_PRODUCTION_MIGRATION_CONFIRMATION_PROPERTY =
  "CUT_HUB_PRODUCTION_SPREADSHEET_ID";
const BOOKING_RATING_PRODUCTION_MIGRATION_LOCK_WAIT_MS = 30000;
const BOOKING_RATING_PRODUCTION_SPREADSHEET_ID_PATTERN = /^[A-Za-z0-9_-]{20,128}$/;
...
function assertBookingRatingProductionMigrationEnvironment() {
  ...
  const confirmedSpreadsheetId = String(
    properties.getProperty(BOOKING_RATING_PRODUCTION_MIGRATION_CONFIRMATION_PROPERTY) || ""
  ).trim();
  ...
  if (
    ...
    !BOOKING_RATING_PRODUCTION_SPREADSHEET_ID_PATTERN.test(configuredSpreadsheetId) ||
    !BOOKING_RATING_PRODUCTION_SPREADSHEET_ID_PATTERN.test(confirmedSpreadsheetId) ||
