...
function previewProductionShadowBehaviorReadOnly() {
  var stage = "identity";
  ...
  try {
    ...
    stage = "phase5";
    requireValue(evaluated.comparison && !evaluated.comparison.phase5Error && phase5Result,
      "PHASE5_DIAGNOSTIC_CALLBACK_FAILED");
    stage = "behavior";
    code = "BEHAVIOR_COMPARISON_FAILED";
    function projectBehavior(result) {
      requireValue(result && Array.isArray(result.slots) &&
        result.slots.every(function (slot) { return typeof slot === "string"; }) &&
        typeof result.availability === "string" && typeof result.shiftStart === "string" &&
        typeof result.shiftEnd === "string", "BEHAVIOR_TYPE_INVALID");
      return {
        slots: result.slots.slice(), availability: result.availability,
        shiftStart: result.shiftStart, shiftEnd: result.shiftEnd
      };
    }
    var legacyBehavior = projectBehavior(legacyResult);
    var phase5Behavior = projectBehavior(phase5Result);
    var fieldEquality = {
      slots: JSON.stringify(legacyBehavior.slots) === JSON.stringify(phase5Behavior.slots),
      availability: legacyBehavior.availability === phase5Behavior.availability,
      shiftStart: legacyBehavior.shiftStart === phase5Behavior.shiftStart,
      shiftEnd: legacyBehavior.shiftEnd === phase5Behavior.shiftEnd
    };
    var slotsAvailabilityEqual = fieldEquality.slots && fieldEquality.availability;
    var output = {
      status: "success", diagnostic: "PRODUCTION_SHADOW_BEHAVIOR_READONLY",
      ...
      legacyBehavior: legacyBehavior, phase5Behavior: phase5Behavior,
      behaviorEqual: slotsAvailabilityEqual && fieldEquality.shiftStart && fieldEquality.shiftEnd,
      slotsAvailabilityEqual: slotsAvailabilityEqual, fieldEquality: fieldEquality,
      behavioralHashes: {
        legacy: BookingAvailabilityPhase5.hash(legacyBehavior),
        phase5: BookingAvailabilityPhase5.hash(phase5Behavior)
      },
    ...
    return { status: "error", diagnostic: "PRODUCTION_SHADOW_BEHAVIOR_READONLY_FAILED",
