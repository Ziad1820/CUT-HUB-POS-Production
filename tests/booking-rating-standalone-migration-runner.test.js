...
const runnerRoot = path.join(root, "scripts", "booking-rating-standalone-migration-runner");
const migrationPath = path.join(runnerRoot, "booking-rating-production-migration-core.js");
const adapterPath = path.join(runnerRoot, "standalone-migration-adapter.js");
const manifestPath = path.join(runnerRoot, "appsscript.json");
const migrationSource = fs.readFileSync(migrationPath, "utf8");
const adapterSource = fs.readFileSync(adapterPath, "utf8");
const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
const PRODUCTION_ID = "synthetic-production-spreadsheet-id-000001";
...
function createHarness(options = {}) {
  ...
  const openByIdCalls = [];
  const spreadsheet = {
    getId: () => options.openedSpreadsheetId || options.spreadsheetId || PRODUCTION_ID,
  ...
  const properties = {
    ...
    CUT_HUB_PRODUCTION_SPREADSHEET_ID: PRODUCTION_ID,
    CUT_HUB_MIGRATION_RUNNER_ENABLED: "true",
    CUT_HUB_MIGRATION_EXECUTION_APPROVED: "true",
    ...(options.properties || {})
  };
  const lock = {
    tryLock: () => {
  ...
  const context = {
    ...
    SpreadsheetApp: {
      openById: (spreadsheetId) => {
        openByIdCalls.push(spreadsheetId);
        if (options.openError) throw options.openError;
        return options.noSpreadsheet ? null : spreadsheet;
      }
    },
    LockService: {
      getScriptLock: () => lock
  ...
  const approvedCore = context.runBookingRatingProductionMigrationCore_;
  let coreDelegations = 0;
  context.runBookingRatingProductionMigrationCore_ = (...args) => {
    coreDelegations += 1;
    return approvedCore(...args);
  };
  return {
    context,
    spreadsheet,
    ...
    openByIdCalls,
    coreDelegations: () => coreDelegations
  };
}
...
test("preview opens only the configured approved production spreadsheet ID", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
    properties: { CUT_HUB_MIGRATION_EXECUTION_APPROVED: null }
  });
  const result = harness.context.previewStandaloneBookingRatingProductionMigration();
  assert.equal(result.status, "dry-run");
  assert.deepEqual(harness.openByIdCalls, [PRODUCTION_ID, PRODUCTION_ID]);
  assert.equal(harness.coreDelegations(), 1);
});

test("standalone sources never use active spreadsheet state", () => {
  const source = `${migrationSource}\n${adapterSource}`;
  assert.doesNotMatch(source, /SpreadsheetApp\.getActive(?:Spreadsheet)?\s*\(/);
  const harness = createHarness({ sheets: migratedSheets() });
  assert.equal(typeof harness.context.SpreadsheetApp.getActive, "undefined");
  assert.equal(typeof harness.context.SpreadsheetApp.getActiveSpreadsheet, "undefined");
  assert.equal(
    harness.context.previewStandaloneBookingRatingProductionMigration().status,
    "dry-run"
  );
});

test("missing environment property fails before opening the spreadsheet", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
    properties: { CUT_HUB_ENVIRONMENT: null }
  });
  assert.throws(
    () => harness.context.previewStandaloneBookingRatingProductionMigration(),
    (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
  );
  assert.deepEqual(harness.openByIdCalls, []);
  assert.equal(harness.lockAcquisitions(), 0);
});

test("non-production environment fails before opening the spreadsheet", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
...
test("missing configured spreadsheet ID fails closed", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
    properties: { CUT_HUB_SPREADSHEET_ID: null }
  });
  assert.throws(
    () => harness.context.previewStandaloneBookingRatingProductionMigration(),
    (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
...
test("wrong configured spreadsheet ID fails before openById", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
    properties: { CUT_HUB_SPREADSHEET_ID: "wrong-production-id" }
  });
  assert.throws(
    () => harness.context.previewStandaloneBookingRatingProductionMigration(),
    (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
...
test("missing, mismatched, and malformed confirmation IDs fail before openById", () => {
  for (const properties of [
    { CUT_HUB_PRODUCTION_SPREADSHEET_ID: null },
    { CUT_HUB_PRODUCTION_SPREADSHEET_ID: "different-production-sheet-id" },
    {
      CUT_HUB_SPREADSHEET_ID: "bad id!",
      CUT_HUB_PRODUCTION_SPREADSHEET_ID: "bad id!"
    }
  ]) {
    const harness = createHarness({ sheets: migratedSheets(), properties });
    assert.throws(
      () => harness.context.previewStandaloneBookingRatingProductionMigration(),
      (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
    );
...
test("runner-enabled property must be explicitly true", () => {
  for (const value of [null, "", "false", "1", "yes"]) {
    const harness = createHarness({
      sheets: migratedSheets(),
      properties: { CUT_HUB_MIGRATION_RUNNER_ENABLED: value }
    });
    assert.throws(
      () => harness.context.previewStandaloneBookingRatingProductionMigration(),
      (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
...
test("preview succeeds without execution approval", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
    properties: { CUT_HUB_MIGRATION_EXECUTION_APPROVED: null }
  });
...
test("execution fails before openById without explicit execution approval", () => {
  for (const value of [null, "", "false", "1", "yes"]) {
    const harness = createHarness({
      sheets: migratedSheets(),
      properties: { CUT_HUB_MIGRATION_EXECUTION_APPROVED: value }
    });
    assert.throws(
      () => harness.context.executeStandaloneBookingRatingProductionMigration(),
      (error) => error.code === "STANDALONE_MIGRATION_EXECUTION_NOT_APPROVED"
    );
    assert.deepEqual(harness.openByIdCalls, []);
    assert.equal(harness.lockAcquisitions(), 0);
  }
...
test("execution delegates to the approved core exactly once when every gate passes", () => {
  const harness = createHarness({ sheets: migratedSheets() });
  const result = harness.context.executeStandaloneBookingRatingProductionMigration();
...
test("opened spreadsheet identity mismatch fails closed", () => {
  const harness = createHarness({
    sheets: migratedSheets(),
    openedSpreadsheetId: "opened-wrong-spreadsheet"
  });
  assert.throws(
    () => harness.context.previewStandaloneBookingRatingProductionMigration(),
    (error) => error.code === "STANDALONE_MIGRATION_SPREADSHEET_IDENTITY_MISMATCH"
  );
  assert.deepEqual(harness.openByIdCalls, [PRODUCTION_ID]);
  assert.equal(harness.lockAcquisitions(), 0);
});

test("standalone project exposes only the two approved operator entry points", () => {
  const source = `${migrationSource}\n${adapterSource}`;
  for (const forbidden of [
    "doGet", "doPost", "onOpen", "onEdit",
    "ScriptApp.newTrigger", "timeBased", "previewBookingRatingProductionMigration",
    "executeBookingRatingProductionMigration"
  ]) {
    assert.doesNotMatch(source, new RegExp(`\\b${forbidden.replace(".", "\\.")}\\b`));
  }
  const publicFunctions = [...adapterSource.matchAll(/^function\s+([A-Za-z0-9_$]+)\s*\(/gm)]
    .map((match) => match[1])
    .filter((name) => !name.endsWith("_"));
  assert.deepEqual(publicFunctions, [
    "previewStandaloneBookingRatingProductionMigration",
    "executeStandaloneBookingRatingProductionMigration"
  ]);
});

test("standalone manifest is minimal and has no Web App configuration", () => {
  assert.equal(manifest.timeZone, "Africa/Cairo");
  assert.equal(manifest.runtimeVersion, "V8");
  assert.equal(manifest.exceptionLogging, "STACKDRIVER");
  assert.equal(Object.prototype.hasOwnProperty.call(manifest, "webapp"), false);
...
test("second execution is idempotent", () => {
  ...
  const first = harness.context.executeStandaloneBookingRatingProductionMigration();
  ...
  const second = harness.context.executeStandaloneBookingRatingProductionMigration();
...
test("configuration mismatch and lock timeout fail closed", () => {
  ...
  assert.throws(
    () => wrongEnvironment.context.previewStandaloneBookingRatingProductionMigration(),
    (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
  );
  ...
  const wrongSpreadsheet = createHarness({
    sheets: migratedSheets(),
    properties: { CUT_HUB_SPREADSHEET_ID: "wrong-configured-id" }
  });
  assert.throws(
    () => wrongSpreadsheet.context.previewStandaloneBookingRatingProductionMigration(),
    (error) => error.code === "STANDALONE_MIGRATION_CONFIGURATION_INVALID"
  );
  ...
  assert.throws(
    () => locked.context.previewStandaloneBookingRatingProductionMigration(),
