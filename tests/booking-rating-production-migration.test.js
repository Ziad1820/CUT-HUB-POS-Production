...
test("configuration mismatch and lock timeout fail closed", () => {
  ...
  for (const properties of [
    ...
    const invalidPin = createHarness({ sheets: migratedSheets(), properties });
    assert.throws(
      () => invalidPin.context.runBookingRatingProductionMigration(true),
      (error) => error.code === "PRODUCTION_MIGRATION_CONFIGURATION_INVALID"
    );
    assert.equal(invalidPin.lockAcquisitions(), 0);
  }
